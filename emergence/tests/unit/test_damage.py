...
class TestArmorReduction(unittest.TestCase):
    ...
    def test_armor_reduces(self):
        r = resolve_damage(6, 0, "kinetic", "neutral", armor=2)
        self.assertEqual(r.after_armor, r.after_affinity - 2)

    def test_armor_floor_zero(self):
        r = resolve_damage(1, 0, "kinetic", "neutral", armor=5)
...
class TestTrackAllocation(unittest.TestCase):
    ...
    def test_basic_physical(self):
        result = allocate_to_tracks(3, "kinetic", {"physical": 1, "mental": 0, "social": 0})
        self.assertEqual(result["track_changes"]["physical"], 3)
        self.assertEqual(result["new_tracks"]["physical"], 4)

    def test_overflow_harm(self):
        result = allocate_to_tracks(4, "kinetic", {"physical": 4, "mental": 0, "social": 0})
    ...
    def test_mental_overflow_to_physical(self):
        result = allocate_to_tracks(6, "cognitive", {"physical": 0, "mental": 3, "social": 0})
    ...
    def test_ec_crit_corruption(self):
        result = allocate_to_tracks(2, "paradoxic", {"physical": 0, "mental": 0, "social": 0}, is_crit=True)
        self.assertEqual(result["corruption"], 1)

    def test_zero_damage(self):
        result = allocate_to_tracks(0, "kinetic", {"physical": 0, "mental": 0, "social": 0})
        self.assertEqual(result["track_changes"], {})


class TestAffinityProfiles(unittest.TestCase):
    """Test AFFINITY_PROFILES have all 6 damage types (Rev 4 broads)."""

    def test_all_profiles_have_six_keys(self):
        for name, profile in AFFINITY_PROFILES.items():
            self.assertEqual(len(profile), 6, f"Profile '{name}' has {len(profile)} keys, expected 6")
